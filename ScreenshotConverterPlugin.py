...
# Version 1.0.7 - Fixed access to EventManager (now uses _event_manager)
...
class ScreenshotConverterPlugin(PluginBase):
    ...
    def handle_screenshot_event(self, event: Event) -> list[ProjectedEvent]:
        ...
        if not filename:
            ...
            projected_events.append(ProjectedEvent({"event": "ScreenshotError", "message": "Screenshot event missing Filename"}))
            return projected_events

        if not self.plugin_helper:
            log("error", "[ScreenshotConverter] PluginHelper not ready.")
            projected_events.append(ProjectedEvent({"event": "ScreenshotError", "message": "Plugin helper not ready"}))
            return projected_events
        ...
        if not bmp_path.exists():
            ...
            projected_events.append(ProjectedEvent({"event": "ScreenshotError", "message": error_message}))
            return projected_events

        current_system = self._get_current_star_system()

        threading.Thread(target=self._convert_screenshot, args=(bmp_path, current_system, event), daemon=True).start()
        return projected_events

    def _get_current_star_system(self) -> str:
        try:
            if self.plugin_helper:
                event_manager = self.plugin_helper._event_manager  # fixed access
    ...
    def _convert_screenshot(self, bmp_path: Path, current_system: str, original_event: Event):
        try:
            if not bmp_path.exists():
                ...
                log("error", f"[ScreenshotConverter] {error_message}")
                if self.plugin_helper:
                    error_event = ProjectedEvent({"event": "ScreenshotError", "message": error_message})
                    self.plugin_helper._event_manager.add_projected_event(error_event, original_event)
                return
            ...
            if self.plugin_helper:
                ...
                self.plugin_helper._event_manager.add_projected_event(success_event, original_event)
            ...
            if self.plugin_helper:
                error_event = ProjectedEvent({"event": "ScreenshotError", "message": error_message, "path": str(bmp_path)})
                self.plugin_helper._event_manager.add_projected_event(error_event, original_event)
# --- END OF FILE ---
